...
class LCDFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        my_image = customtkinter.CTkImage(light_image=Image.open('images/logo.png'),size=(100,13)) # WidthxHeight
...
class buttonsFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.master = master
        self.configure(fg_color="#23272d")
        self.buttonMenu = customtkinter.CTkButton(self, text="Menu",text_color="grey40", command=self.openSetupWindow, width=25, fg_color="grey10", hover_color="grey25")
        self.buttonMenu.grid(row=0, column=0, padx=2, pady=10)
        self.buttonGraph = customtkinter.CTkButton(self, text="Graph",text_color="grey40", command=self.openGraphWindow, width=25, fg_color="grey10", hover_color="grey25")
    ...
    def openGraphWindow(self):
        ...
        ax1.plot(self.master.dataArray[:,0], self.master.dataArray[:,1], 'b', label='Voltage (V)')
        ax1.legend(loc="lower left")
        ax1.axhline(y=self.master.setVoltage)
        ax1.xaxis.set_major_formatter(fmt)
        
        ax2 = plt.subplot(3, 1, 2)
        ax2.plot(self.master.dataArray[:,0], self.master.dataArray[:,2], 'b', label='Current (A)')
        ax2.legend(loc='lower left')
        ax2.xaxis.set_major_formatter(fmt)

        ax3=plt.subplot(3, 1, 3)
        ax3.plot(self.master.dataArray[:,0], self.master.dataArray[:,3], 'b', label='Power (W)')
        ...
        while not self.master.killed:
            ...
            ax1.plot(self.master.dataArray[:,0], self.master.dataArray[:,1], 'b', label='Voltage (V)')
            ax2.plot(self.master.dataArray[:,0], self.master.dataArray[:,2], 'g', label='Current (A)')
            ax3.plot(self.master.dataArray[:,0], self.master.dataArray[:,3], 'r', label='Power (W)')
            ax1.grid()
            ax2.grid()
            ax3.grid()
            ax1.xaxis.set_major_formatter(fmt)
...
class ToplevelWindow(customtkinter.CTkToplevel):
    ...
    def saveSetup(self):
        ...
        f = open ("./param","wb")
        dump(self.master.serialPort,f)
        dump(self.master.baudrate,f)
        dump(self.master.deviceAddress,f)
        f.close()
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        try:
            f = open("./param","rb")
            self.serialPort = load(f)
            self.baudrate = load(f)
            self.deviceAddress = load(f)
            f.close()
        except:
            print('no setup file')
    ...
    def connect(self):
        try:
            self.client = ModbusClient( port=self.serialPort ,framer=FramerType.RTU, baudrate=self.baudrate, timeout=1, parity="N",)
            self.client.connect()
            self.client.read_holding_registers(address=0x00, count=8,slave=self.deviceAddress)
            
        except:
            print('connection error')
            self.knob_frame.ledRx.itemconfig(self.knob_frame.ledRxCircle, fill='red')

    def readData(self):
        try:
            result  = self.client.read_holding_registers(address=0x00, count=8,slave=self.deviceAddress).encode()
            statusBin = "{:08b}".format( int( result[1:2].hex(), 16 ) )
            if statusBin[-1] == "1": self.powerOutOn = True
            else : self.powerOutOn = False
            if statusBin[-2] == "1": self.OCPOn = True
            else : self.OCPOn = False
            if statusBin[-4] == "1": self.endian = "big"
            else : self.endian = "little"
            if statusBin[-5] == "1": self.constantCurrent = True
            else : self.constantCurrent = False
            if statusBin[-6] == "1": self.alarm = True
            else : self.alarm = False
            ...
            self.realVoltage = round(int.from_bytes(result[5:7], self.endian)*decimalVoltage,2)
            self.realCurrent = round(int.from_bytes(result[7:9], self.endian)*decimalCurrent,3)
            
            self.maxVoltage  = round(int.from_bytes(result[13:15], self.endian)*decimalVoltage,2)
            self.maxCurrent  = round(int.from_bytes(result[15:17], self.endian)*decimalCurrent,3)

            if not(self.keyboardLocked):
                self.setVoltage  = round(int.from_bytes(result[9:11], self.endian)*decimalVoltage,2)
                self.setCurrent  = round(int.from_bytes(result[11:13], self.endian)*decimalCurrent,3)
            else :
            ...
            try:
                ...
                if not(self.client.connected):
                    ...
                    if self.alarm: #if there is an alarm
                        ...
                        self.knob_frame.voltageKnob.configure(end=self.maxVoltage,)
                        self.knob_frame.currentKnob.configure(end=self.maxCurrent,)
            except Exception as inst:
                print(inst)
